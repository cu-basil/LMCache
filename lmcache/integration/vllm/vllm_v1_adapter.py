...
class LMCacheConnectorV1Impl:
    ...
    def get_num_new_matched_tokens(
        ...
        try:
            ...
            # Header fallback — only attempted when extra_args path found nothing.
            if not _pesto_id:
                _headers = getattr(request, "headers", None)
                if isinstance(_headers, dict):
                    _pesto_id = _headers.get(
                        "X-PESTO-Request-ID"
                    ) or _headers.get("x-pesto-request-id")
                elif _headers is not None and hasattr(_headers, "get"):
                    _pesto_id = _headers.get("X-PESTO-Request-ID")
